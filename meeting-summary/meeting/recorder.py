"""PipeWire audio capture via parec (PulseAudio).
...
NOTE: use `parec -d <source>` rather than `pw-record --target <source>`:
pw-record's --target resolves the `.monitor` name to the wrong node on this
machine (it captures the mic), while parec taps the real monitor output.
...
class PipeWireRecorder:
    ...
    def __init__(self, on_pcm=None, target: str | None = None,
                 system_internal: bool = True, binary: str = "parec",
