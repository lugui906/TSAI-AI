...
HISTORY_DIR = os.path.expanduser("~/.ainote2")
...
class AiPanel(Gtk.Box):
    ...
    def _build_ui(self):
        ...
        self._history_btn = Gtk.ToggleButton(label="📜")
        self._history_btn.set_tooltip_text("显示历史记录")
        self._history_btn.connect("toggled", self._on_history_toggle)
        header_box.append(self._history_btn)

        self.append(header_box)

        self._history_revealer = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4)
        self._history_revealer.set_visible(False)
        history_header = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=4)
        history_header.set_margin_start(8)
        history_header.set_margin_end(8)
        history_header.set_margin_top(2)
        hl = Gtk.Label(label="历史记录")
        hl.add_css_class("dim-label")
        hl.set_xalign(0)
        hl.set_hexpand(True)
        history_header.append(hl)
        clear_btn = Gtk.Button(label="清空")
        clear_btn.connect("clicked", self._on_history_clear)
        history_header.append(clear_btn)
        self._history_revealer.append(history_header)
        self._history_scrolled = Gtk.ScrolledWindow()
        self._history_scrolled.set_size_request(-1, 140)
        self._history_scrolled.set_vexpand(False)
        self._history_list = Gtk.ListBox()
        self._history_list.set_selection_mode(Gtk.SelectionMode.SINGLE)
        self._history_list.connect("row-activated", self._on_history_activated)
        self._history_scrolled.set_child(self._history_list)
        self._history_revealer.append(self._history_scrolled)
        self.append(self._history_revealer)
        ...
        input_bar.append(self.stop_button)
    ...
    def _on_history_toggle(self, btn):
        if btn.get_active():
            self._refresh_history_ui()
        self._history_revealer.set_visible(btn.get_active())
    ...
    def _refresh_history_ui(self):
        ...
        if not records:
            row = Gtk.ListBoxRow()
            lbl = Gtk.Label(label="暂无历史记录", xalign=0)
        ...
        for rec in records:
            ...
            lbl = Gtk.Label(label=f"{ts}  {title}", xalign=0)
    ...
    def _on_history_activated(self, _listbox, row):
        ...
        while self.message_list.get_first_child() is not None:
        ...
        for msg in rec.get("messages", []):
            ...
            text = msg.get("text", "")
            self._add_message(text, role)
        self._waiting = False
        self._pending_mode = ""
        self.stop_button.set_sensitive(False)

    def _on_history_clear(self, *args):
        save_history([])
        self._refresh_history_ui()

    def _save_current_to_history(self):
        start = getattr(self, "_turn_start", 0)
        msgs = [m for m in self.messages[start:] if m.get("text", "").strip()]
        if not msgs:
            return
        title = ""
        for m in msgs:
    ...
    def _on_stop(self, *args):
        ...
        self.session.cancel()
        self._remove_loading()
        self._add_message("已停止生成", "system")
        self._waiting = False
        self._pending_mode = ""
        self.stop_button.set_sensitive(False)
