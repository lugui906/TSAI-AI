...
GRADIENT_DEFAULT = ("#dceefa", "#fde6ee")  # 天蓝 → 粉色（与 theme.css 浅色一致，比较浅）
GRADIENT_DEFAULT_DARK = ("#22314d", "#45304e")  # 深色模式暗调渐变（与 theme.css 深色一致）


def gradient_css(colors=None, angle=135):
    """生成 GTK CSS 渐变背景（线性渐变）。colors=(起色, 止色)。"""
    c1, c2 = colors if colors else GRADIENT_DEFAULT
    return f"linear-gradient({angle}deg, {c1} 0%, {c2} 100%)"


def pick_gradient(colors=None, dark=None):
    """未指定颜色时按配色模式返回默认渐变（浅色天蓝→粉 / 深色暗调）。"""
    if colors:
        return tuple(colors)
    if dark is None:
        dark = detect_dark_mode()
    return tuple(GRADIENT_DEFAULT_DARK if dark else GRADIENT_DEFAULT)


def apply_gradient_gtk3(colors=None, angle=135, window=None):
    """为 GTK3 窗口叠加渐变背景。返回 CssProvider（可 keep 引用）。"""
    import gi
    gi.require_version("Gtk", "3.0")
    from gi.repository import Gtk, Gdk
    ...
    css = (
        ...
        f"background-image: {gradient_css(pick_gradient(colors), angle)}; "
    ...
    if window is not None:
        window.get_style_context().add_provider(provider, priority)
    else:
        screen = Gdk.Screen.get_default()
        if screen is not None:
            Gtk.StyleContext.add_provider_for_screen(screen, provider, priority)
...
def apply_gradient_gtk4(colors=None, angle=135, window=None):
    """为 GTK4 窗口叠加渐变背景。返回 CssProvider（可 keep 引用）。

    也把内嵌 webview 背景设为透明，供 python+html 应用的渐变穿透。
    """
    import gi
    gi.require_version("Gtk", "4.0")
    gi.require_version("Gdk", "4.0")
