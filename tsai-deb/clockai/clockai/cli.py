...
def cmd_exec(args):
    ...
    task.last_run = datetime.now().replace(second=0, microsecond=0).isoformat()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_exec = sub.add_parser("exec", help="Run a task by id (used by systemd service)")
    p_exec.add_argument("id", help="Task ID")
