...
GrabAccelerator。因此本模块在发起 D-Bus 调用时临时抢占一个当前未被占用的
白名单总线名（如 org.gnome.InitialSetup / org.gnome.Settings）。

注意：总线名只在 GrabAccelerator / UngrabAccelerator 调用期间临时持有，
调用返回后立即 ReleaseName 释放 —— 避免长期占用 org.gnome.Settings
导致系统设置（gnome-control-center）无法通过 D-Bus 激活启动。
"""
import time
...
class AcceleratorManager:
    ...
    def register(self, accelerator, cmd):
        ...
        仅在 GrabAccelerator 调用期间临时持有白名单总线名，调用结束后
        立即释放，避免长期占用 org.gnome.Settings 阻塞系统设置启动。
        """
