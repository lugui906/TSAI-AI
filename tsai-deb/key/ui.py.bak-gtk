...
class AssistantWindow(Gtk.ApplicationWindow):
    def __init__(self, app, start_hidden=False):
        ...
        if not start_hidden:
            self.present()
        self._load_current_on_start()

    def _load_current_on_start(self):
        self._app.backend.current_conversation_id(
            on_done=lambda cid: GLib.idle_add(self._on_start_current, cid),
            on_error=None,
        )

    def _on_start_current(self, cid):
        if cid is not None:
            self._app.new_chat = False
            self.show_conversation_chat(cid)
        return False
    ...
    def _build_header(self):
        ...
        btn_hist = Gtk.Button(label="对话记录")
        btn_hist.connect("clicked", lambda *a: self.open_history())
        hb.pack_start(btn_hist)
    ...
    def _build_body(self):
        ...
        entry_sw.set_size_request(-1, 70)
        ...
        self.btn_stop.connect("clicked", lambda *a: self.stop())
    ...
    def stop(self):
        if not self._app.backend.busy:
            return
        self._app.backend.stop()
        self._stream_text = self._stream_text or "（已停止）"
    ...
    def show_conversation_chat(self, cid):
        self._app.backend.show_conversation(
            cid,
            on_done=lambda conv: GLib.idle_add(self._render_conversation_chat, conv),
            on_error=lambda e: GLib.idle_add(self.stream_error, e),
        )

    def _render_conversation_chat(self, conv):
        self._stream_label = None
        self._stream_text = ""
        self._tree_text = None
        while (child := self.chat_list.get_first_child()) is not None:
        ...
        self.entry.get_buffer().set_text("", -1)
        if conv.get("prompt"):
            self.add_bubble("user", conv["prompt"])
        if conv.get("reply"):
            self.add_bubble("ai", conv["reply"])
        self.entry.grab_focus()
        self.set_status(f"已切换到对话 #{conv.get('id')}，发送消息将延续该对话")
        return False

    def open_history(self):
        if self._history_dlg is None:
            self._history_dlg = ConversationDialog(self._app, self)
            self._history_dlg.connect("close-request", self._on_history_closed)
        self._history_dlg.present()

    def _on_history_closed(self, *_a):
        self._history_dlg = None
        return False
...
class ConversationViewDialog(Gtk.Window):
    def __init__(self, app, conv, parent=None):
        super().__init__(transient_for=parent, modal=True)
        self._app = app
        self.set_title(f"对话 #{conv['id']}（{conv['time']}）")
        ...
        meta = Gtk.Label(label=f"引擎: {conv['engine']}    命令: {conv['command']}    时间: {conv['time']}")
...
class ConversationDialog(Gtk.Window):
    def __init__(self, app, parent=None):
        super().__init__(transient_for=parent, modal=True)
        self._app = app
        self.set_title("对话记录（最新在前，#1 最新）")
    ...
    def _build(self):
        ...
        btn_refresh.connect("clicked", lambda *a: self.refresh())
        btn_view = Gtk.Button(label="查看内容")
        btn_view.connect("clicked", lambda *a: self.view_selected())
        btn_switch = Gtk.Button(label="切换到此对话")
        btn_switch.add_css_class("suggested-action")
        btn_switch.connect("clicked", lambda *a: self.switch_selected())
        bar.append(btn_refresh)
        bar.append(btn_view)
        bar.append(btn_switch)
        vbox.append(bar)

        hint = Gtk.Label(label="双击或选中后点“查看内容”查看完整对话；“切换到此对话”后，发送的消息将延续该对话。")
        hint.set_halign(Gtk.Align.START)
        hint.add_css_class("status")
        vbox.append(hint)
        ...
        self.status = Gtk.Label(label="加载中…")
    ...
    def refresh(self):
        self.set_status("加载对话列表…")
        self._app.backend.list_conversations(
            on_done=lambda items: GLib.idle_add(self._on_listed, items),
            on_error=lambda e: GLib.idle_add(self._on_error, e),
        )
        self._app.backend.current_conversation_id(
            on_done=lambda cid: GLib.idle_add(self._on_current_id, cid),
            on_error=lambda e: GLib.idle_add(self._on_error, e),
        )

    def _on_error(self, msg):
        self.set_status(f"出错了：{msg}")
        return False

    def _on_current_id(self, cid):
    ...
    def render_rows(self):
        ...
        if not self._items:
            row = Gtk.ListBoxRow()
            row.set_child(Gtk.Label(label="暂无对话记录"))
            self.listbox.append(row)
            self.set_status("暂无对话记录")
            return
        for it in self._items:
            mark = "● " if it["id"] == self._current_id else ""
            prompt = (it["prompt"] or "").replace("\n", " ").strip()
            if len(prompt) > 48:
                prompt = prompt[:48] + "…"
            label = Gtk.Label(label=f"{mark}#{it['id']}  {it['time']}  {it['command']}  {prompt}")
            label.set_xalign(0.0)
            ...
            row.set_child(label)
            self.listbox.append(row)
        cur = self._current_id or "无"
        self.set_status(f"共 {len(self._items)} 条对话，当前: {cur}")

    def selected_row(self):
        row = self.listbox.get_selected_row()
        return getattr(row, "conv_id", None)

    def view_selected(self):
        cid = self.selected_row()
        if cid is None:
            self.set_status("请先选择一条对话")
            return
        self.set_status(f"加载对话 #{cid}…")
        self._app.backend.show_conversation(
            cid,
            on_done=lambda conv: GLib.idle_add(self._on_view, conv),
            on_error=lambda e: GLib.idle_add(self._on_error, e),
        )

    def _on_view(self, conv):
        ConversationViewDialog(self._app, conv, parent=self).present()
        self.set_status("")
        return False

    def on_row_activated(self, *_a):
        self.view_selected()
    ...
    def switch_selected(self):
        ...
        if cid is None:
            self.set_status("请先选择一条对话")
            return
        self.set_status(f"切换对话 #{cid}…")
        self._app.backend.change_conversation(
            cid,
            on_done=lambda out: GLib.idle_add(self._on_switched, cid, out),
            on_error=lambda e: GLib.idle_add(self._on_error, e),
        )

    def _on_switched(self, cid, out):
        self._app.new_chat = False
        self.set_status(f"已切换：后续发送将延续对话 #{cid}")
        self._current_id = cid
        self.render_rows()
        self._app.win.show_conversation_chat(cid)
        return False
...
class AssistantApp(Gtk.Application):
    ...
    def do_startup(self):
        ...
        try:
            ...
            if display:
                # 高于 chstyle.apply_gtk4() 的 APPLICATION+1，避免深色模式下
                # 主题的 label { color: #e7eaf0 } 覆盖气泡自身的文字颜色
                #（否则 AI 浅色气泡+浅色文字=内容看不见）
