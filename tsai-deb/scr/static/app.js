(() => {
    ...
    const toast = $('#toast'), historyList = $('#historyList'), autoScroll = $('#autoScroll');
    ...
    function scrollBottom() { if (autoScroll.checked) msgList.scrollTop = msgList.scrollHeight; }
    ...
    function appendStream(t) { if (streamBubble) streamBubble.textContent += t; scrollBottom(); }
    ...
    async function stop() {
        ...
        await api('/api/stop');
        setBusy(false);
        endStream(); setStatus('任务已终止');
    }

    $('#newBtn').onclick = newChat;
    $('#sideNew').onclick = newChat;
    $('#clearBtn').onclick = () => { msgList.innerHTML = ''; streamBubble = null; };
