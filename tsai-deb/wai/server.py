...
def _chindshell_static(filename):
    return send_from_directory("/usr/chindows/chindshell/static", filename)
DATA_DIR = os.path.join(os.path.expanduser("~"), ".wai")
...
# ---------------------- 执行aim命令（增加超时、进程管控） ----------------------
def _file_note(files):
    if not files:
        return ""
    return "\n[上传附件]: " + ", ".join(os.path.basename(str(f)) for f in files)

def run_aim(cmd, sid, for_schedule=False, timeout=300, files=None):
    cwd = get_workdir(sid)
    cmd = list(cmd)
    if files:
        for fp in files:
...
def stop_generation():
    ...
    with running_lock:
        proc = running_procs.pop(sid, None)
...
def chat():
    ...
    with lock:
        ...
        if is_new:
            ...
            sessions[sid].setdefault("history", []).append({
                "role": "user",
                "content": c + _file_note(files)
            })
            save_sessions()
            cmd = ["aim", "run", c]
    return Response(run_aim(cmd, sid, files=files), mimetype="application/x-ndjson")
...
def new_conversation():
    ...
    sid = new_session(c)
    if files or c:
        return Response(run_aim(["aim", "newrun", c], sid, files=files), mimetype="application/x-ndjson")
